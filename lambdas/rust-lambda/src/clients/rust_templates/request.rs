...
impl Request {
    ...
    async fn execute_request(request: hyper::Request<Body>, app: std::sync::Arc<std::sync::Mutex<axum::Router>>) -> Result<(http::StatusCode, hyper::body::Body), Error> {
        let res = {
            let mut guard = app.lock().map_err(|e| Error::Unspecified(format!("Unable to lock {:?}", e)))?;
            let app = guard.deref_mut();
            ServiceExt::<AxumRequest<Body>>::ready(app)
                .await?
                .call(request)
                .await
        }?;
        ...
        let bytes = hyper::body::to_bytes(res.into_body()).await.map_err(|e| Error::from(e))?;
        Ok((status_code, hyper::body::Body::from(bytes)))
    }
            ...
            for<'de> U: serde::Deserialize<'de>,
    {
        ...
        Box::pin(Self::execute_request(request, conf.app.clone())
            .map_err(|e| Error::from(e))
            .and_then(move |response| {
                let status = response.0;
                if !status.is_success() {
                    futures::future::err::<U, Error>(Error::from((status, response.1))).boxed()
                    ...
                    hyper::body::to_bytes(response.1)
